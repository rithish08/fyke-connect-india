...
const EnhancedJobCard: React.FC<JobCardProps> = ({
  ...
  return (
    <div className="bg-white p-3 rounded-xl border border-gray-100 shadow-sm w-full max-w-2xl transition-all duration-200 flex items-start min-h-[96px] gap-3 cursor-pointer">
      ...
      <div className="flex flex-col gap-1 items-end justify-between ml-2">
        <Button
          className="h-8 px-3 rounded-lg text-xs font-semibold shadow border-none outline-none min-w-[60px] bg-gradient-to-tr from-blue-500 via-sky-400 to-indigo-400 text-white hover:scale-105 hover:bg-blue-600"
          onClick={handleApply}
          disabled={showApplied}
        >
          {showApplied ? translateText('common.applied', 'Applied') : `₹${salary} /${translateText("common.apply", "Apply")}`}
        </Button>
      </div>
    </div>
  );
